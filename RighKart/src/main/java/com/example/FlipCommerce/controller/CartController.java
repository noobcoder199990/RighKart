...
public class CartController {
    ...
    @PostMapping("/remove")
    public ResponseEntity removeToCart(@RequestBody RemoveItemFromCartDto removeItemFromCartDto){

        try{
            Item item = itemService.removeItem(removeItemFromCartDto);
            CartResponseDto cartResponseDto = cartService.removeToCart(item,removeItemFromCartDto);
            return new ResponseEntity(cartResponseDto,HttpStatus.ACCEPTED);
        }
        catch (Exception e){
            return new ResponseEntity(e.getMessage(), HttpStatus.BAD_REQUEST);
